...
from loss_visible import calc_sigma, calc_mu_sigma, calc_noise_var, likelihood_loss
...
parser.add_argument("--val_dirs",
                    type=str,
                    default="../dataset/fmdd_sub/validation")
parser.add_argument(
    '--subfold',
    type=str,
    required=True,
...
parser.add_argument("--save_model_path",
                    type=str,
                    default="./experiments/b2s_fmdd")
parser.add_argument("--log_name", type=str, default="xxx_b2s_13rf21_fmdd")
...
parser.add_argument("--w_est", type=float, default=0.0)  # 0.01
...
class DataLoader_Fmdd_sub(Dataset):
    ...
    def __init__(self, data_dir, patch=256):
        ...
        self.train_fns = glob.glob(os.path.join(data_dir, opt.subfold,
                                                'raw/**/**.png'),
                                   recursive=True)
        self.train_fns.sort()
        print('fetch {} samples for training'.format(len(self.train_fns)))
...
def validation_fmdd(dataset_dir):
    ...
    for fn in fns:
        ...
        gt_fn = os.path.join(os.path.dirname(fn.replace('raw', 'gt')),
                             'avg50.png')
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        for valid_name, valid_data in valid_dict.items():
            ...
            for idx in range(num_img):
                ...
                save_path = os.path.join(
                    save_dir,
                    "{}_{:03d}-{:03d}_mu_e.png".format(valid_name, idx, epoch),
                )
                Image.fromarray(mu255_e.squeeze()).save(save_path)
                save_path = os.path.join(
                    save_dir,
                    "{}_{:03d}-{:03d}_mu_m.png".format(valid_name, idx, epoch),
                )
