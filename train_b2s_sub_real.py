...
parser.add_argument(
    ...
    default="pg0.1+0.02",
    choices=[
        "real", "gauss25", "gauss5_50", "poisson30", "poisson5_50", "pg30_25",
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, noisy in enumerate(TrainingLoader):
        ...
        noisy = noisy / 255.0
        noisy = noisy.cuda()
        if not "real" in opt.noisetype:
            noisy = noise_adder.add_train_noise(noisy)
        n, c, h, w = noisy.shape
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for idx, noisy_im in enumerate(valid_images):
                noisy_im = np.array(noisy_im, dtype=np.float32) / 255.0
                if not "real" in opt.noisetype:
                    noisy_im = noise_adder.add_valid_noise(noisy_im)
