        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for idx, im in enumerate(valid_images):
                ...
                with torch.no_grad():
                    ...
                    dn_output = (network(net_input) * mask).view(
                        n, -1, out_chn, h, w).sum(dim=1)
                    # exp
                ...
                # mu_m
                psnr_mu_m = calculate_psnr(origin255.astype(np.float32),
                                           mu255_m.astype(np.float32))
                avg_psnr_mu_m.append(psnr_mu_m)
                ssim_mu_m = calculate_ssim(origin255.astype(np.float32),
                                           mu255_m.astype(np.float32))
                ...
                save_path = os.path.join(
                    save_dir,
                    "{}_{:03d}-{:03d}_mu_m{:.3f}-{:.4f}.png".format(
                        valid_name, idx, epoch, psnr_mu_m, ssim_mu_m),
                )
