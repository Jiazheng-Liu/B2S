...
def calc_covariance(noisy, dn_out, exp_out):
    ...
    if c == 1:
        ...
        A_m1 = torch.zeros(size=(n, h, w, c, c),
                           dtype=A_c1.dtype,
                           device=A_c1.device)
        A_m2 = torch.zeros(size=(n, h, w, c, c),
                           dtype=A_c2.dtype,
                           device=A_c2.device)
        # Calculate A^T * A
        ...
        A_m2 = torch.zeros(size=(n, h, w, c, c),
                           ...
                           device=A_c2.device)
        # Calculate A^T * A
        A_m1[..., [0, 0, 0, 0, 1, 1, 1, 2, 2, 3],
             [0, 1, 2, 3, 1, 2, 3, 2, 3, 3]] = A_c1
        A_m2[..., [0, 0, 0, 0, 1, 1, 1, 2, 2, 3],
             [0, 1, 2, 3, 1, 2, 3, 2, 3, 3]] = A_c2
        sigma_x = torch.matmul(A_m1, A_m2.transpose(-1, -2))  # NHWCC
    return sigma_x


def calc_wb_pme(noisy, mu_x, mu_e, sigma_x, sigma_e, est_out, beta,
                noise_type):
    device = noisy.device
    inc = noisy.shape[1]
    ...
    sigma_xn = noise_var_x.permute(
        0, 2, 3, 1).unsqueeze(-1) * I  # NHWC1 * NHWCC = NHWCC
    noise_var_e = calc_noise_var(est_out, mu_e, noise_type)  # NHWCC
    sigma_en = noise_var_e.permute(
        0, 2, 3, 1).unsqueeze(-1) * I  # NHWC1 * NHWCC = NHWCC
    nx_w = 1 / (1 + beta)
    ne_w = beta / (1 + beta)
...
def calc_sigma(noisy, cov_map):
    ...
    if c == 1:
        ...
        A_m = torch.zeros(size=(n, h, w, c, c),
                          dtype=A_c.dtype,
                          device=A_c.device)
        # Calculate A^T * A
        ...
        A_m = torch.zeros(size=(n, h, w, c, c),
                          ...
                          device=A_c.device)
        # Calculate A^T * A
        A_m[..., [0, 0, 0, 0, 1, 1, 1, 2, 2, 3],
            [0, 1, 2, 3, 1, 2, 3, 2, 3, 3]] = A_c
        sigma_x = torch.matmul(A_m, A_m.transpose(-1, -2))  # NHWCC
    return sigma_x
...
def calc_noise_var(est_out, mu_x, noise_type):
    ...
    if noise_type == "gauss":
        ...
        noise_var = est_out[:, 0:1, :, :]**2 + torch.maximum(
            mu_x, eps) * est_out[:, 1:2, :, :]  # NCHW
...
def likelihood_loss(noisy, mu_x, sigma_x, sigma_n, factor=1, train=True):
    ...
    def batch_vtmv(v, m):  # Batched (v^T * M * v)
        ...
        return (torch.matmul(torch.matmul(v.unsqueeze(-2), m),
                             v.unsqueeze(-1)).squeeze(-1).squeeze(-1))
    ...
    if inc == 1:
        if train:
            ...
            loss_out = (
                (noisy - mu_x)**2) / sigma_y + torch.log(sigma_y)  # N1HW
        else:
            sigma_n = sigma_n  # N111 / N1HW
            pme_out = (noisy * sigma_x + mu_x * sigma_n) / (sigma_x + sigma_n
                                                            )  # N1HW
    else:
        mu_x2 = mu_x.permute(0, 2, 3, 1)  # NHWC
        ...
        if train:
            ...
            dets = torch.maximum(
                zero64,
                dets)  # NHW. Avoid division by zero and negative square roots.
