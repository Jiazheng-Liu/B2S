...
parser.add_argument(
    ...
    choices=[
        ...
        "pg5_50_5_50", "pg0.01+0.0002", "pg0.01+0.02", "pg0.05+0.02",
        "pg0.1+0.02", "pg0.1+0.0002", "pg0.05+0.0002"
    ],
)
...
parser.add_argument("--log_name", type=str, default="est_pg0.01+0.02_SNEMI3D")
...
class AugmentNoise(object):
    ...
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            lam, std = self.params
            print(f'alpha, sigma: {self.params}')
            if not self.div:
                lam = 1. / lam
            lam = lam * torch.ones((shape[0], 1, 1, 1), device=x.device)
            std = std * torch.ones((shape[0], 1, 1, 1), device=x.device)
...
# Network
out_chn = 2
estimator = est_UNet(num_classes=out_chn, in_channels=opt.n_channel, depth=3)
if opt.parallel:
...
if opt.checkpoint is not None:
    estimator = load_network(opt.checkpoint, estimator, strict=True)
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, clean in enumerate(TrainingLoader):
        ...
        sigma = est_map[:, 0:out_chn // 2, :, :]
        alpha = est_map[:, out_chn // 2:out_chn, :, :]

        # size: 1, num: 1
        gat_noisy = gat(noisy, sigma, alpha)
        # size: 3/4, ovlp: 1/2, num: 4
        gat_noisy_sub = torch.cat(
            (gat_noisy[:, :, 0:3 * h // 4,
             ...
             gat_noisy[:, :, h // 4:h, 0:3 * w // 4], gat_noisy[:, :, h // 4:h,
                                                                w // 4:w]),
            dim=1)
        # size: 1/2, ovlp: 1/4, num: 9
        gat_noisy_sub_1 = torch.cat(
            (gat_noisy[:, :, 0:h // 2,
                       0:w // 2], gat_noisy[:, :, h // 4:3 * h // 4, 0:w // 2],
             gat_noisy[:, :, h // 2:h, 0:w // 2], gat_noisy[:, :, 0:h // 2,
                                                            w // 4:3 * w // 4],
             gat_noisy[:, :, h // 4:3 * h // 4, w // 4:3 * w // 4],
             gat_noisy[:, :, h // 2:h,
                       w // 4:3 * w // 4], gat_noisy[:, :, 0:h // 2, w // 2:w],
             gat_noisy[:, :, h // 4:3 * h // 4,
                       w // 2:w], gat_noisy[:, :, h // 2:h, w // 2:w]),
            dim=1)

        try:
            est_val_sub_1 = chen_estimate(gat_noisy_sub_1)
        ...
        est_diff_cg = (est_val - 1.0)**2
        est_diff_fg1 = torch.mean((est_val_sub - 1.0)**2, dim=1, keepdim=True)
        est_diff_fg2 = torch.mean((est_val_sub_1 - 1.0)**2,
                                  dim=1,
                                  keepdim=True)
        est_diff = est_diff_cg + est_diff_fg1 + 0.5*est_diff_fg2
        # est_diff = est_diff_cg + est_diff_fg1 + 0.5*est_diff_fg2
        print(f"est_diff: {est_diff.data}")
        loss_est = torch.mean(est_diff)

        loss_all = loss_est

        loss_all.backward()
        # clip the gradient norm of estimator
        total_norm_E = nn.utils.clip_grad_norm_(param_E, clip_grad_E)
        if torch.isnan(total_norm_E):
            continue
        grad_norm_E = (grad_norm_E * (ii / (ii + 1)) + total_norm_E / (ii + 1))
        optimizer.step()
        logger.info(
            "{:04d} {:05d} Loss_Est={:.6f}, Loss_All={:.6f}, Time={:.4f}, GE:{:.1e}/{:.1e}\n sigma={}, alpha:{}"
            .format(
                epoch,
                ii,
                loss_est.item(),
                ...
                clip_grad_E,
                total_norm_E,
                sigma.squeeze().cpu().data.numpy(),
                alpha.squeeze().cpu().data.numpy(),
            ))
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for idx, im in enumerate(valid_images):
                ...
                pred_sigma = sigma.cpu().data.numpy().squeeze(0)
                pred_alpha = alpha.cpu().data.numpy().squeeze(0)
                avg_sigma.append(pred_sigma)
                avg_alpha.append(pred_alpha)

            avg_sigma = np.mean(avg_sigma)
            avg_alpha = np.mean(avg_alpha)

            log_path = os.path.join(validation_path,
                                    "A_log_{}.csv".format(valid_name))
            with open(log_path, "a") as f:
                f.writelines("epoch:{},sigma:{:.6f},alpha:{:.6f}\n".format(
                    epoch,
                    avg_sigma,
                    avg_alpha,
                ))
