...
from nutils.gutils import chen_estimate, gat
...
parser.add_argument(
    ...
    default="gauss25",
    choices=[
        "gauss25", "gauss5_50", "poisson30", "poisson5_50", "pg30_25",
        "pg5_25_5_25", "pg5_25_25_50", "pg25_50_25_50", 
        "pg0.01+0.02", "pg0.05+0.02", "pg0.1+0.02"
    ],
)
...
parser.add_argument("--log_name", type=str, default="b2s_g25_13rf21_SNEMI3D")
...
class AugmentNoise(object):
    ...
    def __init__(self, style):
        ...
        if style.startswith("gauss"):
            ...
            if '_' in style:
                ...
                self.div = False
                self.params = [
                    float(p) for p in style.replace("pg", "").split("+")
                ]
            if len(self.params) == 2:
                self.params[
                    1] = self.params[1] / 255.0 if self.div else self.params[1]
                self.style = "pg_fix"
            elif len(self.params) == 4:
                self.params[
                    2] = self.params[2] / 255.0 if self.div else self.params[2]
                self.params[
                    3] = self.params[3] / 255.0 if self.div else self.params[3]
                self.style = "pg_range"
    ...
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            std = (torch.rand(size=(shape[0], 1, 1, 1), device=x.device) *
            ...
            lam = (torch.rand(size=(shape[0], 1, 1, 1), device=x.device) *
                   (max_lam - min_lam) + min_lam)
            noised = torch.poisson(lam * x, generator=get_generator()) / lam
            return noised
    ...
    def add_valid_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            # Poisson Gaussian Noise
            noised = np.array(np.random.poisson(lam * x) / lam,
                              dtype=np.float32)
            noise = np.array(np.random.normal(size=shape) * std,
                             dtype=np.float32)
            out = noised + noise
            return out
        elif self.style == "pg_range":
...
def generate_mask(img, width=4, mask_type="random"):
    ...
    if mask_type == "random":
        ...
        index = torch.from_numpy(np.array(index).astype(np.int64)).type(
            torch.int64)
    ...
    mask = depth_to_space(
        mask.type_as(img).view(n, h // width, w // width,
                               width**2).permute(0, 3, 1, 2),
        block_size=width,
    ).type(torch.int64)
...
def interpolate_mask(tensor, mask, mask_inv):
    ...
    filtered_tensor = torch.nn.functional.conv2d(tensor.view(n * c, 1, h, w),
                                                 kernel,
                                                 stride=1,
                                                 padding=1)
...
class DataLoader_sRGB(Dataset):

    def __init__(self, data_dir, patch=256):
        super(DataLoader_sRGB, self).__init__()
...
class DataLoader_GrayScale(Dataset):

    def __init__(self, data_dir, patch=256):
        super(DataLoader_GrayScale, self).__init__()
        self.data_dir = data_dir
        self.patch = patch
        self.train_fns = glob.glob(os.path.join(data_dir, '*'))
        self.train_fns.sort()
        print('fetch {} samples for training'.format(len(self.train_fns)))
...
def validation_srgb(dataset_dir):
...
def validation_grayscale(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))
    fns.sort()
    images = []
    for fn in fns:
        im = Image.open(fn)
        im = np.array(im, dtype=np.float32)[:, :, np.newaxis]
        images.append(im)
    return images
...
# Training Set
TrainingDataset = DataLoader_GrayScale(opt.data_dir, patch=opt.patchsize)
...
# Validation Set
SEM_dir = opt.val_dirs
valid_dict = {
    "SEM": validation_grayscale(SEM_dir),
}
...
network = UNet(in_channels=opt.n_channel,
               ...
               wf=opt.n_feature)
estimator = est_UNet(num_classes=3, in_channels=opt.n_channel, depth=3)
if opt.parallel:
...
if opt.resume is not None:
    epoch_init, optimizer, scheduler = resume_state(opt.resume, optimizer,
                                                    scheduler)
    epoch_init += 1
...
if opt.checkpoint is not None and opt.resume is None:
    ...
    for i in range(1, epoch_init):
        ...
        logger.info("----------------------------------------------------")
        logger.info(
            "==> Resuming Training with learning rate:{}".format(new_lr))
        logger.info("----------------------------------------------------")
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    if Lambda <= Thread1:
        ...
        beta = Lambda2 + (Lambda - Thread1) * (increase_ratio -
                                               Lambda2) / (Thread2 - Thread1)
    else:
        beta = increase_ratio
    alpha = Lambda1
    ...
    network.train()
    estimator.train()
    for ii, clean in enumerate(TrainingLoader):
        st = time.time()
        clean = clean / 255.0
        clean = clean.cuda()
        ...
        est_sigma = torch.sqrt(
            (est_sigma_x + beta**2 * est_sigma_e) / (1 + beta)**2)
        ...
        gat_noisy_sub = torch.cat(
            (gat_noisy[:, :, 0:3 * h // 4,
                       0:3 * w // 4], gat_noisy[:, :, 0:3 * h // 4, w // 4:w],
             gat_noisy[:, :, h // 4:h, 0:3 * w // 4], gat_noisy[:, :, h // 4:h,
                                                                w // 4:w]),
            dim=1)
        try:
        ...
        dn_output = (network(net_input) * mask).view(n, -1, out_chn, h,
                                                     w).sum(dim=1)
        ...
        try:
            loss_brv = likelihood_loss(noisy,
                                       mu_m,
                                       sigma_m,
                                       sigma_n,
                                       train=True)
        ...
        loss_all = loss_brv + loss_est  # 20,5,100

        loss_all.backward()
        # clip the gradient norm of network
        ...
        logger.info(
            ...
            .format(
                ...
                clip_grad_D,
                total_norm_D,
                est_sigma.squeeze().cpu().data.numpy(),
                est_alpha.squeeze().cpu().data.numpy(),
            ))
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        save_network(network, epoch, "est")
        ...
        for valid_name, valid_images in valid_dict.items():
            ...
            for idx, im in enumerate(valid_images):
                ...
                with torch.no_grad():
                    ...
                    sigma_n = calc_noise_var(est_map, mu_e, noise_type)
                    sigma_e = (sigma_e + beta**2 * sigma_e) / (1 + beta)**2

                    pme_e = likelihood_loss(noisy_im,
                                            mu_e,
                                            sigma_e,
                                            sigma_n,
                                            train=False)
                ...
                pme_e = pme_e.permute(0, 2, 3, 1)

                mu_e = mu_e.cpu().data.clamp(0, 1).numpy().squeeze(0)
                pme_e = pme_e.cpu().data.clamp(0, 1).numpy().squeeze(0)

                mu255_e = np.clip(mu_e * 255.0 + 0.5, 0, 255).astype(np.uint8)
                pme255_e = np.clip(pme_e * 255.0 + 0.5, 0,
                                   255).astype(np.uint8)

                # calculate psnr
                # mu_e
                psnr_mu_e = calculate_psnr(origin255.astype(np.float32),
                                           mu255_e.astype(np.float32))
                avg_psnr_mu_e.append(psnr_mu_e)
                ssim_mu_e = calculate_ssim(origin255.astype(np.float32),
                                           mu255_e.astype(np.float32))
                avg_ssim_mu_e.append(ssim_mu_e)
                # pme_e
                psnr_pme_e = calculate_psnr(origin255.astype(np.float32),
                                            pme255_e.astype(np.float32))
                avg_psnr_pme_e.append(psnr_pme_e)
                ssim_pme_e = calculate_ssim(origin255.astype(np.float32),
                                            pme255_e.astype(np.float32))
                avg_ssim_pme_e.append(ssim_pme_e)
                ...
                save_path = os.path.join(
                    save_dir,
                    "{}_{:03d}-{:03d}_mu_e{:.3f}-{:.4f}.png".format(
                        valid_name, idx, epoch, psnr_mu_e, ssim_mu_e),
                )
                Image.fromarray(mu255_e.squeeze()).save(save_path)
                save_path = os.path.join(
                    save_dir,
                    "{}_{:03d}-{:03d}_pme_e{:.3f}-{:.4f}.png".format(
                        valid_name, idx, epoch, psnr_pme_e, ssim_pme_e),
                )
                Image.fromarray(pme255_e.squeeze()).save(save_path)

            avg_psnr_mu_e = np.array(avg_psnr_mu_e)
            avg_psnr_mu_e = np.mean(avg_psnr_mu_e)
            avg_ssim_mu_e = np.mean(avg_ssim_mu_e)

            avg_psnr_pme_e = np.array(avg_psnr_pme_e)
            avg_psnr_pme_e = np.mean(avg_psnr_pme_e)
            avg_ssim_pme_e = np.mean(avg_ssim_pme_e)

            log_path = os.path.join(validation_path,
                                    "A_log_{}.csv".format(valid_name))
            with open(log_path, "a") as f:
                f.writelines(
                    "epoch:{},mu_e:{:.3f}/{:.4f}, pme_e:{:.3f}/{:.4f}\n".
                    format(
                        epoch,
                        avg_psnr_mu_e,
                        avg_ssim_mu_e,
